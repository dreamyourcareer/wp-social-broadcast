...
<div id="collect_container">
						...
						for($i=0, $n=count($friends); $i<$n; ++$i)
						{
							...
							<tr>
								<td class="checkbox-container">
									<input type="checkbox" value="<?php WP_Social_Broadcast::getValue($provider, $friends[$i]);?>" name="friend[]"/> 
								</td>
								<td class="user-img">
								...
								<td class="last-child">
									<?php WP_Social_Broadcast::printName($friends[$i]->displayName);?>
  ...
  <?php if ( !isset($_GET['wsi_hook']) || $_GET['wsi_hook'] != 'anyone') : ?>
   <div class="fields-wrapper">
   		...
   		<?php WP_Social_Broadcast::printSubjectField($provider, $settings);?>
  
  
  		<?php WP_Social_Broadcast::printMessageField($provider, $settings);?>
