<?php
/*
Plugin Name: WP Social Broadcast
Plugin URI: http://www.WPSocialbroadcast.com
Description: This plugin will provide your site with a unique possibility to viral spreading the news you provide on social networks such as Twitter, Facebook, Linkedin, Google, Yahoo, Hotmail and more.
Version: 1.0.0
Author: Karl Lillrud & Navnish Bhardwaj
Author URI: http://www.WPSocialbroadcast.com
...
@ session_start();
$_SESSION["wsi::plugin"] = "WordPress Social Broadcast ";
...
class WP_Social_Broadcast extends WP_Plugin_Base_free
{
	...
	function __construct() {
		...
		$this->WPB_PREFIX		=	'wsi';
		self::$PREFIX			=	'wsi';
		$this->WPB_SLUG			=	'wp-social-broadcast'; // Need to match plugin folder name
		$this->WPB_PLUGIN_NAME	=	'Wordpress Social Broadcast';
		$this->WPB_VERSION		=	'1.0.0';
		...
		//hook to handle accepted broadcast
		add_action( 'register_form', array(&$this, 'process_broadcast' ));
		add_action( 'bp_before_register_page', array(&$this, 'bp_process_broadcast' ));
	...
	function admin_notice(){
		...
		?>
	    <div class="error">
	    	<h3>Wordpress Social Broadcast</h3>
	        <p><?php _e(sprintf('Please go to the <a href="%s">settings page</a> and check the new settings and default messages. | <a href="%s">Hide Notice</a>', '?page=wp-social-broadcast','?wsi-dismiss=yes'),$this->WPB_PREFIX);?></p>
	    </div>
	    <?php
		}
	} 
	...
	*/
	function activate()
	{
		...
		$wpdb->query("CREATE TABLE IF NOT EXISTS `".$wpdb->base_prefix."wsi_queue` (
				  ...
				  `i_count` INT NULL COMMENT 'Quantity of Broadcast',
	...
	*/
	...
	{
		add_menu_page( 'WP Social Broadcast', 'WP Social Broadcast', 'manage_options', $this->WPB_SLUG ,array(&$this, 'display_page') );
	...
	*/
	...
	?>
		...
		<div class="info-box">
		<h2><?php printf(__('Welcome to <a href="%s">Wordpress Social Broadcast</a>!',$this->WPB_PREFIX),'http://www.WPSocialbroadcast.com');?></h2>
		<p><?php printf(__('To start using the plugin you need to fill out the OAuth settings of the following providers. If you need help, please read the <a href="%s" target="_blank">documentation</a> or go to the <a href="%s" target="_blank">support forum</a>.',$this->WPB_PREFIX),'http://www.WPSocialbroadcast.com/docs/','http://www.WPSocialbroadcast.com/');?></p>
		<p><?php _e('You can place the invitation widget on any page by using the following shortcode:',$this->WPB_PREFIX);?></p>
		<code>[wsi-widget title="Do Social Broadcast"]</code>
		<p>Or you can place it in your templates with:</p>
		<code>WP_Social_Broadcast::widget('Some title');</code>
		
		<p><?php echo sprintf(__('If you have any question please carefully <a href="%s">read documentation</a> before opening a ticket',$this->WPB_PREFIX), 'http://www.WPSocialbroadcast.com/docs');?></p>
		<h2 style="font-weight:bold;">Premium Features <a href="http://www.WPSocialbroadcast.com"><span style="font-size:50%;color:red">(Get Premium here)</span></a></h2>
		
		<ul>
<li>* Unlimited usage not limited for 30 days as in Free version</li>
<li>* GMAIL SUPPORT</li>
<li>* Redirect users after they send broadcast</li>
<li>* Free Support</li>
		</ul>
	...
	*/
	...
	?>
		...
		<div class="info-box">
		<h2><?php printf(__('So far %s broadcast were sent using WP Social Broadcast!',$this->WPB_PREFIX),$total_invites);?></h2>
		<br>
	...
	*/
	...
	?>
		...
		<p><?php echo sprintf(__('If you have any question please carefully <a href="%s">read the documentation</a> before opening a ticket',$this->WPB_PREFIX), 'http://www.WPSocialbroadcast.com');?></p>
	...
	*/
	...
	?>
		...
		<div id="gmail" style="display:none">
			<p><?php _e('Easy to setup. Limited to 500 emails a day. We recommend that you open a dedicated Gmail account for this purpose.',$this->WPB_PREFIX); echo  '<a href="http://www.WPSocialbroadcast.com/" style="color:red">(Premium Only)</a>';?></p>
		</div>
	
		<div id="smtp" style="display:none">
			<p><?php _e('Send with a professional SMTP provider, a great choice for big sites',$this->WPB_PREFIX); echo '<a href="www.WPSocialbroadcast.com" style="color:red">(Premium Only)</a>';?></p>
		</div>
		<script type="text/javascript">
		jQuery(document).ready(function($) { 
	...
	*/
	...
	?>
		<div class="info-box">
			...
			<p><?php echo sprintf(__('If you have any question please carefully <a href="%s">read documentation</a> first opening a ticket',$this->WPB_PREFIX), 'http://www.WPSocialbroadcast.com/docs');?></p>
		</div><?php
	}
	...
	*/
	...
	{
		$wsi = WP_Social_Broadcast::get_instance();
	...
	*/
	...
	/**
	 * Function to process broadcast
	 *
	 */
	 
	 function handle_user_broadcast(){
...
/**
	 * Function to process broadcast in BP
	 *
	 */
	 
	 function bp_handle_user_broadcast(){
	...
	*/
	function process_login_auth() {
		...
		if( ! empty( $provider ) && ($provider == 'live' || $provider == 'mail' || $hybridauth->isConnectedWith( $provider ) ))
		{
			...
			<title><?php _e('Select your Friends',$this->WPB_PREFIX);?> - Wordpress Social Broadcast</title>
			...
			<div id="footer">
				<div id="credits">
					Powered by <a href="http://www.WPSocialbroadcast.com" target="_blank">WP Social Broadcast</a>
				</div>
				<script type="text/javascript">
				jQuery(function($) { 
	...
	*/
	...
	/**
	* AJAX that add broadcast to queue
	*/
...
function wsi_shortcode_func($atts){
	...
	ob_start();
	WP_Social_Broadcast::widget($title);
